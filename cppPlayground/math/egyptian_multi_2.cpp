...
template <int N> bool odd(N);
template <int N> N half(N);
...
template <int N> bool odd(N number) { return number & 0x1; }
template <int N> N half(N number) { return number >> 1; }

template <typename T> T neutral_element_add(std::plus<T>) { return T(0); }
template <typename T> T neutral_element_multi(std::multiplies<T>) {
  return T(0);
}
