...
int main() {
  ...
  unsigned long po;
  int index = 0;
  std::cout << "Press A to add elelment, D to display, P to lift up to "
               "powerand K to end"
            << std::endl;
  ...
  while (std::cin >> ch && std::toupper(ch) != 'K') {
    ...
    std::cout << "Press A to add elelment, D to display, P to lift up to power "
                 "and K to end"
              << std::endl;
  }
}
...
void power_element(unsigned long *element) { *element = pow(*element, 2); }